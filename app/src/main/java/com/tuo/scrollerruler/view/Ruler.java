...
//        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
    ...
    private void initPaints() {
        ...
        mSmallScalePaint.setColor(Color.GRAY);
        mSmallScalePaint.setStrokeCap(Paint.Cap.ROUND);
        ;

        mBigScalePaint = new Paint();
        mBigScalePaint.setColor(Color.GRAY);
...
//        drawScale(canvas);
        ...
        canvas.drawText("foisdhfoioieioeoifowqdoiqdfoiqfoiqfoiqwndodnasncoiqwioerhqwiorhfqwoinqwoncqoiwoiuhqwhfoisdhfoioieioeoifowqdoiqdfoiqfoiqfoiq", 0f, 90f, paint);
    }

    //画刻度和字
    private void drawScale(Canvas canvas) {
        for (float i = 40; i <= 2200; i++) {
            float locationX = (i - 40 * 18);
            ...
            if (locationX > getScrollX() - mDrawOffset && locationX < (getScrollX() + canvas.getWidth() + mDrawOffset)) {
                if (i % mCount == 0) {
                    ...
                    canvas.drawText(String.valueOf(i / 10), locationX, 120, mTextPaint);
                } else {
    ...
    private float scrollXtoScale(int scrollX) {
        return ((float) (scrollX - mMinPositionX) / mLength) * mMaxLength + 40;
    }

    //把Scale转化为ScrollX
    private int scaleToScrollX(float scale) {
        return (int) ((scale - 40) / mMaxLength * mLength + mMinPositionX);
    }
